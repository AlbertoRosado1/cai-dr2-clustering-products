...
def compute_auw(imock):
    ...
    zrange = (1.1, 1.6)
    data = tools.prepare_catalog(tools.read_catalog(kind='data', **kw_catalog), kind='data', zrange=zrange, **kw_catalog)
    randoms = tools.prepare_catalog(tools.read_catalog(kind='randoms', expand=expand, **kw_catalog), kind='randoms', zrange=zrange, **kw_catalog)
    binned_weight = {}
    ...
    parent_data = tools.prepare_catalog(tools.read_catalog(kind='full_data', **kw_catalog, concatenate=True), kind='parent_data', **kw_catalog, binned_weight=binned_weight)
    ...
    result['GK'] = get_counts(lambda: {'data': complete_data}, lambda: {'data': kappamap})
    result['GcGc'] = get_counts(lambda: {'data': complete_data}, lambda: {'data': complete_data})
    result['GG'] = get_counts(lambda: {'data': data}, lambda: {'data': data})
    result['RcRc'] = get_counts(lambda: {'data': complete_randoms}, lambda: {'data': complete_randoms})
    result['RR'] = get_counts(lambda: {'data': randoms}, lambda: {'data': randoms})
