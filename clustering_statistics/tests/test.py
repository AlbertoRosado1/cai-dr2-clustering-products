...
def test_catalog_blinding_adapter_uses_desiblind(monkeypatch):
    calls = []

    class FakeTracerCatalogBlinder:
        @staticmethod
        def get_blinding_modes(modes):
            calls.append(("modes", modes))
            return ("bao", "fnl")

        @staticmethod
        def from_options(options, tracer="LRG"):
            calls.append(("params", options, tracer))
            return {"modes": ("bao", "fnl"), "metadata": "sealed", "options": options}

        @staticmethod
        def apply_bao_blinding_to_catalogs(catalogs, params, zcol="Z"):
            calls.append(("bao", catalogs, params, zcol))
            return catalogs

        @staticmethod
        def apply_rsd_blinding(data, randoms, params, tracer="LRG"):
            calls.append(("rsd", data, randoms, params, tracer))
            return data

        @staticmethod
        def apply_fnl_blinding(data, randoms, params, tracer="LRG"):
            calls.append(("fnl", data, randoms, params, tracer))
            return data, randoms

        @staticmethod
        def output_version(version, params):
            calls.append(("output", version, params))
            return f"{version}-bao-fnl-blinded"

        @staticmethod
        def output_version_from_options(version, options):
            calls.append(("output_options", version, options))
            return f"{version}-bao-fnl-blinded"

        @staticmethod
        def blinding_attrs(params):
            calls.append(("attrs", params))
            return {"catalog_blinding": "bao,fnl", "catalog_blinding_metadata": "sealed"}

    monkeypatch.setattr(catalog_blinding, "TracerCatalogBlinder", FakeTracerCatalogBlinder)
    options = {"modes": ["ap", "png"], "parameters_fn": "secret.npy"}
    params = catalog_blinding.get_blinding_parameters(options, tracer="ELG_LOPnotqso")
    catalog = ToyCatalog(Z=np.array([0.5]))
    assert catalog_blinding.get_blinding_modes(options) == ("bao", "fnl")
    assert catalog_blinding.apply_bao_blinding_to_catalogs(catalog, params) is catalog
    assert catalog_blinding.output_version("data-dr2-v2", params) == "data-dr2-v2-bao-fnl-blinded"
    assert ("params", options, "ELG_LOPnotqso") in calls
    assert any(call[0] == "bao" for call in calls)


def test_catalog_blinding_compute_suppresses_data_vector_blinding(monkeypatch, tmp_path):
    import clustering_statistics.compute_stats as compute_stats_module

    calls = []
    written = []

    def fake_get_blinding_parameters(options, tracer="LRG", mpicomm=None):
        calls.append(("params", tracer, dict(options)))
        return {"modes": ("bao", "rsd", "fnl"), "metadata": "sealed", "options": dict(options)}

    def fake_apply_bao(catalogs, params, zcol="Z"):
        calls.append(("bao", isinstance(catalogs, list), params["modes"]))
        return catalogs

    def fake_apply_rsd(data, randoms, params, tracer="LRG"):
        calls.append(("rsd", tracer))
        data.attrs["catalog_blinding_stage"] = "rsd"
        return data

    def fake_apply_fnl(data, randoms, params, tracer="LRG"):
        calls.append(("fnl", tracer))
        data.attrs["catalog_blinding_stage"] = "fnl"
        return data, randoms

    def fake_read_catalog(kind, **kwargs):
        calls.append(("read", kind, kwargs.get("version"), kwargs.get("region")))
        catalog = ToyCatalog(
            Z=np.array([0.5, 0.6]),
    ...
    def fake_prepare_catalog(catalog, kind, **kwargs):
        ...
        if isinstance(catalog, list):
            return catalog
        catalog.attrs.setdefault("prepared", kind)
        return catalog

    def fake_get_stats_fn(kind, catalog=None, **kwargs):
        version = catalog["LRG"]["version"] if isinstance(catalog, dict) else catalog["version"]
        return tmp_path / f"{kind}_{version}.npy"

    def fake_write_stats(filename, stats, mpicomm=None):
        written.append((filename, dict(stats.attrs)))

    def fake_compute_mesh2_spectrum(*args, **kwargs):
        calls.append(("compute_mesh2_spectrum",))
        return {"raw": ToyStat()}

    monkeypatch.setattr(catalog_blinding, "get_blinding_parameters", fake_get_blinding_parameters)
    monkeypatch.setattr(catalog_blinding, "apply_bao_blinding_to_catalogs", fake_apply_bao)
    monkeypatch.setattr(catalog_blinding, "apply_rsd_blinding", fake_apply_rsd)
    monkeypatch.setattr(catalog_blinding, "apply_fnl_blinding", fake_apply_fnl)
    monkeypatch.setattr(catalog_blinding, "output_version", lambda version, params: f"{version}-bao-rsd-fnl-blinded")
    monkeypatch.setattr(
        catalog_blinding,
        "blinding_attrs",
        lambda params: {"catalog_blinding": "bao,rsd,fnl", "catalog_blinding_metadata": "sealed"} if params else {},
    )
    monkeypatch.setattr(tools, "check_if_stats_requires_blinding", lambda **kwargs: True)
    monkeypatch.setattr(tools, "apply_blinding", mock.Mock(side_effect=AssertionError("data-vector blinding should be suppressed")))
    monkeypatch.setattr(tools, "write_stats", fake_write_stats)
    monkeypatch.setattr(compute_stats_module, "compute_mesh2_spectrum", fake_compute_mesh2_spectrum)
    ...
    compute_stats_from_options(
        ...
        catalog=dict(
            ...
            region="NGC",
            nran=1,
            blinding=dict(modes=["bao", "rsd", "fnl"], parameters_fn="secret.npy", metadata="sealed"),
        ),
        mesh2_spectrum={},
    )

    assert tools.apply_blinding.call_count == 0
    assert any(call[0] == "bao" for call in calls)
    assert ("rsd", "LRG") in calls
    assert ("fnl", "LRG") in calls
    assert written
    assert "bao-rsd-fnl-blinded" in str(written[0][0])
    assert written[0][1]["catalog_blinding"] == "bao,rsd,fnl"


def test_catalog_blinding_saved_catalog_dry_run_is_sealed(monkeypatch, tmp_path, caplog):
    from clustering_statistics.job_scripts import desipipe_catalog_blinding as job

    monkeypatch.setattr(job.tools, "propose_fiducial", lambda kind, tracer, analysis: {"nran": 2, "FKP_P0": 10000.0})

    def fake_get_catalog_fn(kind, **kwargs):
        filename = tmp_path / kwargs.get("cat_dir", "planned") / f"{kind}_{kwargs.get('region', 'ALL')}.fits"
        if kind == "randoms":
            return [filename]
        return filename

    monkeypatch.setattr(job.tools, "get_catalog_fn", fake_get_catalog_fn)
    monkeypatch.setattr(
        job.catalog_blinding,
        "get_blinding_parameters",
        lambda options, tracer="LRG", mpicomm=None: {
            "modes": ("bao", "fnl"),
            "metadata": "sealed",
            "w0": -0.99,
            "wa": 0.05,
    ...
    monkeypatch.setattr(
        job.catalog_blinding,
        "blinding_attrs",
        lambda params: {"catalog_blinding": "bao,fnl", "catalog_blinding_metadata": "sealed"},
    )

    with caplog.at_level(logging.INFO):
        output = job.make_blinded_catalogs(
            tracer="LRG",
            version="data-dr2-v2",
            output_root=tmp_path,
            regions=("NGC",),
            modes=("bao", "fnl"),
            parameters_fn="secret.npy",
            maxr=1,
            dry_run=True,
        )

    assert "data-dr2-v2-bao-fnl-blinded" in output
    assert "secret.npy" not in caplog.text
    assert "-0.99" not in caplog.text
    assert "7.0" not in caplog.text
